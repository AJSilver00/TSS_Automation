    ...
    def open_job(self, jobref):
        while self.findref == False:
            ...
            if findref != None:
                ...
                TSSallhirescreen = self.pyautogui.locateOnScreen('TSSallhirescreen.png', confidence=0.8)
                if TSSallhirescreen != None:
                    ...
                    TSSicon = self.pyautogui.locateOnScreen('TSSicon.png', confidence=0.8)
                    TSSicon = self.pyautogui.center(TSSicon)
                    x, y = TSSicon
                    self.pyautogui.click(x, y)
                    email().allhiretab()
        ...
        while self.searchref == False:
            highlightref = self.pyautogui.locateOnScreen('highlightref.png', confidence=0.8)
            if highlightref != None:
                ...
                while self.jobfound == False:
                    firmbooking = self.pyautogui.locateOnScreen('firmbooking.png', confidence=0.9)
                    quotebooking = self.pyautogui.locateOnScreen('quotebooking.png', confidence=0.9)
                    if firmbooking != None:
                        print('This job is in Firm')
                        firmbooking = self.pyautogui.center(firmbooking)
                        x, y = firmbooking
                        ...
                        print(
                            'Job {} has now loaded and have double-clicked on it. Lets now wait for it to open'.format(
                                jobref))
                    elif quotebooking != None:
                        print('THIS JOB IS IN QUOTE!!!')
                        quotecontinue = input('Reply YES if you want to continue')
                        if quotecontinue == 'YES':
                            quotebooking = self.pyautogui.center(quotebooking)
                            x, y = quotebooking
                            self.pyautogui.moveTo(x, y)
                            self.pyautogui.click(clicks=2)
                            self.jobfound = True
...
class reminder:
    def sendproforma(self, jobref):
        email().openjob(jobref)
        email().r_proforma()
